...
/*   Updated: 2023/09/22 16:13:18 by orakib           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	Bureaucrat::executeForm(const AForm &object) {
	if (this->grade <= object.getGrade_toexec() && object.getSignature()) {
		object.execute(*this);
		std::cout << this->name << " executed " << object.getName() << std::endl;
	}
	else if (!object.getSignature())
		std::cout << this->name << " couldn't execute " << object.getName()
